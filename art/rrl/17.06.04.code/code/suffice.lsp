  ...
  (prog ()
    ...
    (cond ((null $rule-set)
	   (princ "Note: Rule set is empty, run Kb please.") (terpri) 
					;added 93
	   (princ "; Note: Rule set is empty, run Kb please." $log-port) 
	   (terpri $log-port) (return nil))
	  ((constructors-check $operlist)
					;added 93
	   (princ "No constructors for that type. Stop.") 
	   (terpri)
	   (princ "; No constructors for that type. Stop." $log-port) 
	   (terpri $log-port)
	   (return nil))
          ((null (get-noncons))
	   ...
	   (terpri) 
					;added 93
	   (princ "; Note: All operators are constructors." $log-port) 
	   (terpri $log-port)
	   (return t)))
	...
	;added 93
	(terpri $log-port) 
	(princ "; Note: the system may be not canonical." $log-port) 
	(terpri $log-port))
    (setq  $reduce_time 0
...
(defun is-partial-op (op)
  ...
  (and (test-one-op op $type-testset)
       (when-art-with-input-from-terminal
  ...
  (let ((def-domain (get-def-domain op)) stones)
    (if* (null (cdr def-domain)) 
     then (terpri) (princ (uconcat "No definition for the operator '" op "'."))
	  (terpri)
	  ;added 93
	  (princ (uconcat "; No definition for the operator '" op "'.") $log-port) 
	  (terpri $log-port)
	  (setq stones t)
	  ...
	  (cond
	    (stones 
	     ...
	     ;added 93
	     (terpri $log-port) 
	     (princ (uconcat "; Specification of '" op "' is not known to be completely defined.") $log-port)
	     (terpri $log-port) 
		 ...
		 (terpri)
		 ;added 93
		 (princ "; Following left hand sides of rules are proposed: " $log-port)
		 (terpri $log-port) 
		 (sloop for t1 in stones do 
		   (princ "    ") (write-term t1) (princ " ---> something") (terpri)
		   ;added 93
		   (princ ";    " $log-port) 
		   (write-term t1 $log-port) 
		   (princ " ---> something" $log-port) (terpri $log-port))))
	    (t (terpri)
	       ...
	       ;added 93
	       (terpri $log-port) 
	       (princ (uconcat "; Specification of '" op "' is completely defined.") $log-port) 
	       (terpri $log-port)))
	  stones)))
...
(defun constructors-check (ops &aux flag)
  ...
  (sloop for op in ops 
	...
	do (terpri) (princ (uconcat "Warning: '" op "' is untyped.")) 
	   (terpri)
	   ;added 93
	   (terpri $log-port) (princ (uconcat "; Warning: '" op "' is untyped." $log-port))
	   (terpri $log-port)
	...
	do (princ (uconcat "Note: Constructors for Type '"
           ...
           (princ (uconcat "; Note: Constructors for Type '"
			   type "' is empty." $log-port)) (terpri $log-port)
