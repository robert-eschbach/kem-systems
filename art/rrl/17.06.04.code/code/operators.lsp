...
(defun display-constructors (type-ops &optional port)
  (terpri port)
  (princ "The system has the following constructors:" port) (terpri port)
  (if* (cddr type-ops) 
      then (sloop for ty-ops in (cdr (reverse type-ops)) do
		 (princ (uconcat "     Type '" (car ty-ops) "': "
				 (display-ops (cdr ty-ops))) port) (terpri port))
      else (setq type-ops (car type-ops))
      (princ (uconcat "     Type '" (car type-ops) "': "
		      (display-ops (cdr type-ops))) port) 
      (terpri port)))
