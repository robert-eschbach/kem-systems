...
(defun trace-succ-prove (eqn num)
  ...
  (terpri $log-port) (princ "; All subgoals are proved so " $log-port) ;added 93
  (terpri $log-port) (princ "; " $log-port) (write-comment-eqn eqn $log-port)
  (princ ";  is an inductive theorem." $log-port)
  (terpri $log-port))
