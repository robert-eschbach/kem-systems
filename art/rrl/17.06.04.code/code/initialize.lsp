...
(defun reset-rrl ()
  ...
  ;; Eschbach Mon Jan 20 13:02:12 MET 1997 
  ;; 'r2e
...
"     Undo, Unlog, Write or Help."))
    ...
    (selectq					
      (choose-str (read-atom 'none)
		  '(add akb auto break clean cover delete disable enable help grammar history init
...
(defun order-eqns (&optional dis &aux l2 rule)
  ...
  (if* (or $ac $commutative) then
       ;added 93
     (setq $eqn-set (sloop for eq in $eqn-set collect (flatten-eqn (normalize eq)))) 
     (setq $post-set (sloop for eq in $post-set collect (flatten-eqn (normalize eq)))))
...
(defun close-log ()
  (if* $log-port then
    ...
    (close $log-port) (setq $log-port (outfile "/dev/null"))
