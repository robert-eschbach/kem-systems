...
(defun prove ()
  ...
  (when-art-with-input-from-terminal
    ...
    (when (not new)
	  (terpri $log-port)
	  (princ "; Following equation " $log-port) (terpri $log-port)
	  (princ ";    " $log-port) (write-comment-eqn eqn $log-port)
	  (princ "; is an equational theorem in the system." $log-port) (terpri $log-port))

    ;; Check if induction works.
    ;added 93
    (when-art-with-input-from-terminal
    (when new
...
(defun uncondi-prove (eqn)
  (if* (null (setq eqn (checkeq-normal eqn))) then
      ...
      (princ "; Yes, it is an equational theorem." $log-port) 
      (terpri $log-port) nil
     else 
     (when (> $trace_flag 1)
	   (if $confluent
		 ...
		 ;; added 93
		 (princ "; No, it is not an equational theorem." $log-port))
...
(defun fail-end-induc (&optional eqns &aux done)
   ...
   (princ "    ") (write-eqn $prove-eqn)
					;added 93
   (princ "; Your equation" $log-port) (terpri $log-port)
   (princ ";    " $log-port) (write-comment-eqn $prove-eqn $log-port)
   ...
   (if* $induc then
     ...
     (terpri $log-port) 
					...
					;added 93
     (princ ";    is not an inductive theorem in the system." $log-port) 
     (terpri $log-port))
   (if* (null $induc) then
