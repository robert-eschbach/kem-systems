...
(defun print-normalized-eqn (eqn0 eqn &optional num)
  ...
  (when (and (> $trace_flag 1) (or $used-rule-nums (null eqn) (null (car eqn))))
	(terpri)
;added 93
	(cond ((cdr $used-rule-nums) 
	       (princ "By rules ") 
	       ;(princ "; By rules " $log-port)
	       )
	      ($used-rule-nums 
	       (princ "By rule ") 
	       ;(princ "; By rule " $log-port)
	       )
	      ((not (and eqn (car eqn))) 
	       (princ "By reformulation, ") 
	       ;(princ "; By reformulation, " $log-port)
	       ))
;added 93
	(dolist (num (reverse $used-rule-nums)) 
		(princ "[")    ; (princ "[" $log-port) 
		(princ num)    ; (princ num $log-port)  
		(princ "], ")  ; (princ "], " $log-port)
		)
	(when (and eqn (null (car eqn)))
	      (princ "and by switching the head and the premises, ")
	      ;(princ "and by switching the head and the premises, " $log-port)
	      )
	(terpri)            ; (terpri $log-port)
	(princ "    ")     ; (princ ";    " $log-port)
	(write-seq-eqn num eqn0) ; (write-f-eqn eqn0 $log-port)
	(princ "     is reduced to") ; (princ ";     is reduced to" $log-port)
	(if (null (cdr (ctx eqn))) (setq eqn (change-ctx eqn nil)))
	(if* (car eqn) 
	     then (terpri) (princ "    ") (write-seq-eqn num eqn) 
	        ;  (terpri $log-port) (princ ";    " $log-port) (write-f-eqn eqn $log-port) 
	     else (princ " true.") (terpri) 
	     ; (princ " true." $log-port) (terpri $log-port)
	     )
	)
  )
...
(trace norm-term)(defun cover-norm-term (term &aux new) 
...
(defun cover-condi-norm (eqn &aux head help)
  ; Return 
  ;   1. nil if equation is reduced to identity or one premise is reduced 
  ;      to false.
  ;   2. an reduced equation
  ;   3. a "nil" cons to a reduced equation if one of premises can be reduced
  ...
  ;;added 93
  (when (handle-and?)
	(setq help $premises-set $premises-set nil $premises-set (handle-and help))) 
