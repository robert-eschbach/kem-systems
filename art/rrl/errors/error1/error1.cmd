...
;; Eschbach
;; Hier koennte man [149] anwenden
;;  [149] TIMELIST(APPEND(Z, Z1)) ---> (TIMELIST(Z) * TIMELIST(Z1))  if  
;;        { not((TIMELIST(Z) = 0)),
;;          not((TIMELIST(Z1) = 0)) }   [GENE, 19]
;; aber RRL kann ofensichtlich nicht aus X = TIMELIST(Z) und 
;; not((0 = X)) folgern, dass not((0 = TIMELIST(Z)) gilt.
