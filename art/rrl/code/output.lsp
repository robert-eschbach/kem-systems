...
(defun writef-sys (&aux port)
  (user-selectq
    ...
    (equations-only 
	  ...
	  (display-type-arity $operlist port) (terpri port)
    ...
    (rules-only 
      ...
      (if* (setq port (open-write-file "eqn" t)) then
	  (display-type-arity $operlist port) (terpri port)
	  (write-eqns $rule-set port) (close port)
    ...
    (spec-write 
      "Both equations and rules and ordering. "
      (defvar $spec-port );changed 93
      (if* (setq $spec-port (open-write-file "spec" t)) then
          (display-type-arity $operlist $spec-port) (terpri $spec-port)
	  (display-constructors $type-constructors $spec-port)
          (display-op-stats $spec-port) (terpri $spec-port)
	  (write-eqns (append $eqn-set $post-set) $spec-port) (terpri $spec-port) 
	  (write-rules $rule-set $spec-port) 
	  (close $spec-port)
	  (princ "System written in the file.") (terpri)))
    (kb-status
...
(defun write-f-eqn (eqn &optional port flag)
  ...
  ;added 93
  (when (and (is-assertion eqn) (is-source-type eqn 'def))
	(princ " := true " port))
  (terpri port))
...
(defun write-f-rhs (rhs ctx &optional port (comment nil))
  (if* (truep rhs) then (princ 'true port) else (write-term-simple rhs port))
  (if* ctx then (princ " if " port) 
      (if* (is-premise-set ctx) 
	  then (write-f-premises (cdr ctx) port comment)
...
(defun write-f-premises (pres &optional port comment)
  ;added 93
  (if (equal port $spec-port)
      (write-premises pres port)
    (progn 
      (if* (cdr pres) then
	   (terpri port)
	   ;added 93
	   (if comment (princ ";         (" port)
	     (princ "          (" port))
	   (write-one-pre (car pres) port)
	   (sloop for xa in (cdr pres) do
		  (princ ") and " port) 
